...
use thirtyfour::screen::{By, ByRoleOptions, CurrentState, Screen, TextMatch, ValueOptions};
...
fn test_name_exact_match(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let result =
            screen.get(By::role_with_options("button".to_string(), options_partial.clone())).await;
...
fn test_selected_option(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let tab = screen.get(By::role_with_options("tab".to_string(), options.clone())).await?;
        assert_eq!(tab.text().await?, "Active Tab");

        // Test find_by_role_with_options
        let find_tab = screen.find(By::role_with_options("tab", options.clone())).await?;
        assert_eq!(find_tab.text().await?, "Active Tab");

        // Test get_all_by_role_with_options
        let tabs = screen.get_all(By::role_with_options("tab", options.clone())).await?;
        assert_eq!(tabs.len(), 1);
        assert_eq!(tabs[0].text().await?, "Active Tab");

        // Test query_by_role_with_options
        let maybe_tab = screen.query(By::role_with_options("tab", options.clone())).await?;
        assert!(maybe_tab.is_some());
        assert_eq!(maybe_tab.unwrap().text().await?, "Active Tab");

        // Test query_all_by_role_with_options
        let query_tabs = screen.query_all(By::role_with_options("tab", options.clone())).await?;
        assert_eq!(query_tabs.len(), 1);
        assert_eq!(query_tabs[0].text().await?, "Active Tab");

        // Test find_all_by_role_with_options
        let find_tabs = screen.find_all(By::role_with_options("tab", options.clone())).await?;
...
fn test_checked_checkbox(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let checkbox =
            screen.get(By::role_with_options("checkbox".to_string(), options.clone())).await?;
        let aria_label = checkbox.attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Enable notifications");

        // Test get_all_by_role_with_options
        let checkboxes = screen.get_all(By::role_with_options("checkbox", options.clone())).await?;
        ...
        // Test query_by_role_with_options
        let maybe_checkbox =
            screen.query(By::role_with_options("checkbox", options.clone())).await?;
        assert!(maybe_checkbox.is_some());
        let aria_label = maybe_checkbox.unwrap().attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Enable notifications");

        // Test query_all_by_role_with_options
        let query_checkboxes =
            screen.query_all(By::role_with_options("checkbox", options.clone())).await?;
        assert_eq!(query_checkboxes.len(), 1);
        let aria_label = query_checkboxes[0].attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Enable notifications");

        // Test find_by_role_with_options
        let find_checkbox = screen.find(By::role_with_options("checkbox", options.clone())).await?;
        let aria_label = find_checkbox.attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Enable notifications");

        // Test find_all_by_role_with_options
        let find_checkboxes =
            screen.find_all(By::role_with_options("checkbox", options.clone())).await?;
        assert_eq!(find_checkboxes.len(), 1);
        let aria_label = find_checkboxes[0].attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Enable notifications");
...
fn test_pressed_button(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        // Test query_all_by_role_with_options
        let buttons = screen.query_all(By::role_with_options("button", options.clone())).await?;
        assert_eq!(buttons.len(), 1);
        assert_eq!(buttons[0].text().await?, "Bold");

        // Test get_all_by_role_with_options
        let all_buttons = screen.get_all(By::role_with_options("button", options.clone())).await?;
...
fn test_busy_loading(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let status =
            screen.get(By::role_with_options("status".to_string(), options.clone())).await?;
        assert_eq!(status.text().await?, "Loading...");

        // Test find_all_by_role_with_options
        let statuses = screen.find_all(By::role_with_options("status", options.clone())).await?;
        assert_eq!(statuses.len(), 1);
        assert_eq!(statuses[0].text().await?, "Loading...");

        // Test get_all_by_role_with_options
        let all_statuses = screen.get_all(By::role_with_options("status", options.clone())).await?;
        assert_eq!(all_statuses.len(), 1);
        assert_eq!(all_statuses[0].text().await?, "Loading...");

        // Test query_by_role_with_options
        let maybe_status = screen.query(By::role_with_options("status", options.clone())).await?;
        assert!(maybe_status.is_some());
        assert_eq!(maybe_status.unwrap().text().await?, "Loading...");

        // Test query_all_by_role_with_options
        let query_statuses =
            screen.query_all(By::role_with_options("status", options.clone())).await?;
        assert_eq!(query_statuses.len(), 1);
        assert_eq!(query_statuses[0].text().await?, "Loading...");

        // Test find_by_role_with_options
        let find_status = screen.find(By::role_with_options("status", options.clone())).await?;
...
fn test_current_page(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let link = screen.get(By::role_with_options("link".to_string(), options.clone())).await?;
        assert_eq!(link.text().await?, "Home");

        // Test query_by_role_with_options
        let maybe_link = screen.query(By::role_with_options("link", options.clone())).await?;
        assert!(maybe_link.is_some());
        assert_eq!(maybe_link.unwrap().text().await?, "Home");

        // Test get_all_by_role_with_options
        let links = screen.get_all(By::role_with_options("link", options.clone())).await?;
        assert_eq!(links.len(), 1);
        assert_eq!(links[0].text().await?, "Home");

        // Test query_all_by_role_with_options
        let query_links = screen.query_all(By::role_with_options("link", options.clone())).await?;
        assert_eq!(query_links.len(), 1);
        assert_eq!(query_links[0].text().await?, "Home");

        // Test find_by_role_with_options
        let find_link = screen.find(By::role_with_options("link", options.clone())).await?;
        assert_eq!(find_link.text().await?, "Home");

        // Test find_all_by_role_with_options
        let find_links = screen.find_all(By::role_with_options("link", options.clone())).await?;
...
fn test_hidden_elements(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let _buttons =
            screen.get(By::role_with_options("button".to_string(), options_hidden.clone())).await;
        ...
        let visible_button = screen
            .get(By::role_with_options("button".to_string(), visible_options.clone()))
            .await?;
        assert_eq!(visible_button.text().await?, "Visible Button");

        // Test query_all_by_role_with_options
        let visible_buttons =
            screen.query_all(By::role_with_options("button", visible_options.clone())).await?;
        assert_eq!(visible_buttons.len(), 1);
        assert_eq!(visible_buttons[0].text().await?, "Visible Button");

        // Test get_all_by_role_with_options
        let all_visible_buttons =
            screen.get_all(By::role_with_options("button", visible_options.clone())).await?;
        assert_eq!(all_visible_buttons.len(), 1);
        assert_eq!(all_visible_buttons[0].text().await?, "Visible Button");

        // Test query_by_role_with_options
        let maybe_visible_button =
            screen.query(By::role_with_options("button", visible_options.clone())).await?;
        assert!(maybe_visible_button.is_some());
        assert_eq!(maybe_visible_button.unwrap().text().await?, "Visible Button");

        // Test find_by_role_with_options
        let find_visible_button =
            screen.find(By::role_with_options("button", visible_options.clone())).await?;
        assert_eq!(find_visible_button.text().await?, "Visible Button");

        // Test find_all_by_role_with_options
        let find_visible_buttons =
            screen.find_all(By::role_with_options("button", visible_options.clone())).await?;
...
fn test_level_heading(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let heading =
            screen.get(By::role_with_options("heading".to_string(), options.clone())).await?;
        assert_eq!(heading.text().await?, "Section Title");

        // Test find_all_by_role_with_options
        let headings = screen.find_all(By::role_with_options("heading", options.clone())).await?;
        assert_eq!(headings.len(), 1);
        assert_eq!(headings[0].text().await?, "Section Title");

        // Test get_all_by_role_with_options
        let all_headings =
            screen.get_all(By::role_with_options("heading", options.clone())).await?;
        assert_eq!(all_headings.len(), 1);
        assert_eq!(all_headings[0].text().await?, "Section Title");

        // Test query_by_role_with_options
        let maybe_heading = screen.query(By::role_with_options("heading", options.clone())).await?;
        assert!(maybe_heading.is_some());
        assert_eq!(maybe_heading.unwrap().text().await?, "Section Title");

        // Test query_all_by_role_with_options
        let query_headings =
            screen.query_all(By::role_with_options("heading", options.clone())).await?;
        assert_eq!(query_headings.len(), 1);
        assert_eq!(query_headings[0].text().await?, "Section Title");

        // Test find_by_role_with_options
        let find_heading = screen.find(By::role_with_options("heading", options.clone())).await?;
...
fn test_value_min(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let options =
            ByRoleOptions::new().value(value_opts).name(TextMatch::Exact("Volume".to_string()));

        let slider =
            screen.get(By::role_with_options("slider".to_string(), options.clone())).await?;
        let aria_label = slider.attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Volume");

        // Test find_by_role_with_options
        let find_slider = screen.find(By::role_with_options("slider", options.clone())).await?;
        let aria_label = find_slider.attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Volume");
        // Test get_all_by_role_with_options
        let all_sliders = screen.get_all(By::role_with_options("slider", options.clone())).await?;
        assert_eq!(all_sliders.len(), 1);
        let aria_label = all_sliders[0].attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Volume");

        // Test query_by_role_with_options
        let maybe_slider = screen.query(By::role_with_options("slider", options.clone())).await?;
        ...
        // Test query_all_by_role_with_options
        let query_sliders =
            screen.query_all(By::role_with_options("slider", options.clone())).await?;
        assert_eq!(query_sliders.len(), 1);
        let aria_label = query_sliders[0].attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Volume");

        // Test find_all_by_role_with_options
        let find_sliders =
            screen.find_all(By::role_with_options("slider", options.clone())).await?;
        assert_eq!(find_sliders.len(), 1);
        let aria_label = find_sliders[0].attr("aria-label").await?.unwrap_or_default();
        assert_eq!(aria_label, "Volume");
...
fn test_value_now(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        // Test get_all_by_role_with_options
        let sliders = screen.get_all(By::role_with_options("slider", options.clone())).await?;
...
fn test_query_fallbacks(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        // Test query_all_by_role_with_options
        let elements = screen.query_all(By::role_with_options("checkbox", options.clone())).await?;
        assert_eq!(elements.len(), 1);
        assert_eq!(elements[0].text().await?, "Toggle Switch");

        // Test get_all_by_role_with_options
        let all_elements =
            screen.get_all(By::role_with_options("checkbox", options.clone())).await?;
        ...
        // Test find_by_role_with_options
        let find_element = screen.find(By::role_with_options("checkbox", options.clone())).await?;
