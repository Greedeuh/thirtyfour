...
use thirtyfour::screen::{ByAltTextOptions, Screen, Selector};
...
fn test_by_alt_text_exact_option(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let element = screen
            .get(Selector::alt_text_with_options("Logo".to_string(), exact_options.clone()))
            .await?;
        ...
        let result =
            screen.query(Selector::alt_text_with_options("Logo", exact_options.clone())).await?;
        ...
        let find_elements =
            screen.find_all(Selector::alt_text_with_options("Logo", exact_options.clone())).await?;
