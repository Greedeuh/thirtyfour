...
use thirtyfour_testing_library_ext::{By, ByTestIdOptions, Screen};
...
fn test_by_test_id_exact_option(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let element =
            screen.get(By::test_id_with_options("save".to_string(), exact_options.clone())).await?;
        assert_eq!(element.id().await?, Some("testid-exact".to_string()));

        // Test get_all_by_test_id_with_options
        let elements =
            screen.get_all(By::test_id_with_options("save", exact_options.clone())).await?;
        assert_eq!(elements.len(), 1);
        assert_eq!(elements[0].id().await?, Some("testid-exact".to_string()));

        // Test query_by_test_id_with_options
        let result = screen.query(By::test_id_with_options("save", exact_options.clone())).await?;
        assert!(result.is_some());
        assert_eq!(result.unwrap().id().await?, Some("testid-exact".to_string()));

        // Test query_all_by_test_id_with_options
        let query_elements =
            screen.query_all(By::test_id_with_options("save", exact_options.clone())).await?;
        ...
        let find_element =
            screen.find(By::test_id_with_options("save", exact_options.clone())).await?;
        assert_eq!(find_element.id().await?, Some("testid-exact".to_string()));

        // Test find_all_by_test_id_with_options
        let find_elements =
            screen.find_all(By::test_id_with_options("save", exact_options.clone())).await?;
