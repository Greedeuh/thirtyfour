...
fn find_by_text(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let screen = Screen::load_with_testing_library(c.clone()).await?;
        let span = screen.find_by_text("some text to find").await?;
...
fn find_all_by_text(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let screen = Screen::load_with_testing_library(c.clone()).await?;
        let spans = screen.find_all_by_text("some text to find").await?;
...
fn find_by_text_should_fail(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let screen = Screen::load_with_testing_library(c.clone()).await?;
        let result = screen.find_by_text("NonExistentText").await;
...
fn find_all_by_text_should_fail(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let screen = Screen::load_with_testing_library(c.clone()).await?;
        let result = screen.find_all_by_text("NonExistentText").await;
