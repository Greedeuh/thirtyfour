...
fn test_selector_option_with_id(test_harness: TestHarness) -> WebDriverResult<()> {
    block_on(async {
        ...
        let element = screen
            .get(By::label_text("Email Address").selector("#email-input"))
            .await?;
        ...
        let maybe_element = screen
            .query(By::label_text("Email Address").selector("#email-input"))
            .await?;
        ...
        let find_element = screen
            .find(By::label_text("Email Address").selector("#email-input"))
            .await?;
...
fn test_exact_false_partial_match(test_harness: TestHarness) -> WebDriverResult<()> {
    block_on(async {
        ...
        // Test query_all_by_label_text_with_options
        let query_passwords = screen
            .query_all(By::label_text("Password").exact(false))
            .await?;
        assert_count(&query_passwords, 2)?;

        // Test find_all_by_label_text_with_options
        let find_passwords = screen
            .find_all(By::label_text("Password").exact(false))
            .await?;
...
fn test_exact_case_sensitivity(test_harness: TestHarness) -> WebDriverResult<()> {
    block_on(async {
        ...
        let country_element = screen.get(By::label_text("Country").exact(true)).await?;
        ...
        let country_code_element = screen
            .get(By::label_text("COUNTRY CODE").exact(true))
            .await?;
        assert_eq!(
        ...
        let lowercase_result = screen.query(By::label_text("country").exact(true)).await?;
        ...
        let lowercase_code_result = screen
            .query(By::label_text("country code").exact(true))
            .await?;
