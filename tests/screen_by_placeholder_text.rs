...
fn test_by_placeholder_text_success(test_harness: TestHarness) -> WebDriverResult<()> {
    block_on(async {
        ...
        let elements = screen.get_all(By::placeholder_text("jean@email.fr")).await?;
        ...
        let query_elements = screen.query_all(By::placeholder_text("jean@email.fr")).await?;
        ...
        let find_elements = screen.find_all(By::placeholder_text("jean@email.fr")).await?;
...
fn test_by_placeholder_text_failure(test_harness: TestHarness) -> WebDriverResult<()> {
    block_on(async {
        ...
        let get_result = screen.get(By::placeholder_text("NonExistentPlaceholder")).await;
        assert_error(get_result)?;

        let get_all_result = screen.get_all(By::placeholder_text("NonExistentPlaceholder")).await;
        assert_error(get_all_result)?;

        let find_result = screen.find(By::placeholder_text("NonExistentPlaceholder")).await;
        assert_error(find_result)?;

        let find_all_result = screen.find_all(By::placeholder_text("NonExistentPlaceholder")).await;
        assert_error(find_all_result)?;

        let query_result = screen.query(By::placeholder_text("NonExistentPlaceholder")).await?;
        assert_none(query_result)?;

        let query_all_result = screen.query_all(By::placeholder_text("NonExistentPlaceholder")).await?;
