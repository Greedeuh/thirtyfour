...
fn test_by_test_id_exact_option(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let result = screen.query_by_test_id_with_options("save", &exact_options).await?;
        ...
        let find_element = screen.find_by_test_id_with_options("save", &exact_options).await?;
        assert_eq!(find_element.id().await?, Some("testid-exact".to_string()));

        // Test find_all_by_test_id_with_options
        let find_elements = screen.find_all_by_test_id_with_options("save", &exact_options).await?;
