...
fn test_by_text_exact_option(test_harness: TestHarness) -> WebDriverResult<()> {
    ...
    block_on(async {
        ...
        let result = screen.query_by_text_with_options("Login", &exact_options).await?;
        assert!(result.is_some());
        assert_eq!(result.unwrap().id().await?, Some("text-exact".to_string()));

        // Test query_all_by_text_with_options
        let query_elements = screen.query_all_by_text_with_options("Login", &exact_options).await?;
        ...
        let find_element = screen.find_by_text_with_options("Login", &exact_options).await?;
        assert_eq!(find_element.id().await?, Some("text-exact".to_string()));

        // Test find_all_by_text_with_options
        let find_elements = screen.find_all_by_text_with_options("Login", &exact_options).await?;
